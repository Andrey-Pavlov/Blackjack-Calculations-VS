...
#pragma region Dealer: Constructors and Destructors
...
double Dealer::conditionalSplitWt(double wt,int i,Deck &deck,int splitCard,int kout)
{	double ntot=(double)deck.getTotalCards()+1.;			// number before card i was removed
	wt*=ntot/(ntot-kout);							// input wt = n(i)/ntot, now wt = n(i)/(ntot-k)
	if(i!=splitCard)
	{	double nj=(double)deck.getNumber(splitCard);
		wt*=(ntot-nj-(double)kout)/(ntot-nj);
	}
	return wt;
}
...
#pragma region Dealer: Accessors
