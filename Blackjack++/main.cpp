...
int main (int argc, char * const argv[])
{
	...
	double clockTime=(clock()-startTicks)/CLOCKS_PER_SEC;
	...
	{	dealer.setUpcard(upcard,theDeck);
		...
		*/
		...
		{	if(verbose)
			...
			{	os << c1 ;
				for(c2=1;c2<=c1;c2++)
				{	double standVal=1.5,hitVal,ddVal,splitVal;
		...
		*/
		...
		{	if(verbose)
			...
			double ndecks=theDeck.getDecks();
			...
			{	// mean
				...
				double mean = 0.5*hand.doubleExval(theDeck,dealer)-hand.standExval(theDeck,dealer);
				os << c1 << "\t" << 100.*mean ;
				
				for(c2=ACE;c2<=TEN;c2++)
				{	// remove one card
					theDeck.remove(c2);
					double exval = 0.5*hand.doubleExval(theDeck,dealer)-hand.standExval(theDeck,dealer);
			...
			{	// mean
				hand.reset(1,c1-11);
				double mean = hand.hitExval(theDeck,dealer)-hand.standExval(theDeck,dealer);
				...
				{	// remove one card
					theDeck.remove(c2);
					double exval = hand.hitExval(theDeck,dealer)-hand.standExval(theDeck,dealer);
					os << "\t" << 100.*(exval-mean)*ndecks ;
					theDeck.restore(c2);
				}
			...
			{	// mean
				hand.reset(5,c1-5);
				double mean = hand.doubleExval(theDeck,dealer)-hand.hitExval(theDeck,dealer);
				if(mean < minExval) continue;
				os << c1 << "\t" << 100.*mean ;
				...
				{	// remove one card
					theDeck.remove(c2);
					double exval = hand.doubleExval(theDeck,dealer)-hand.hitExval(theDeck,dealer);
					os << "\t" << 100.*(exval-mean)*ndecks ;
					theDeck.restore(c2);
				}
			...
			{	// mean
				...
				double alt = hit ? hand.hitExval(theDeck,dealer) : hand.standExval(theDeck,dealer) ;
				double mean = hand.doubleExval(theDeck,dealer)-alt;
				if(mean < minExval) continue;
				os << "A," << c1-11 << "\t" << 100.*mean ;
				...
				{	// remove one card
					...
					double exval = hand.doubleExval(theDeck,dealer)-alt;
					os << "\t" << 100.*(exval-mean)*ndecks ;
					theDeck.restore(c2);
				}
			...
			{	bool handResplit = c1==ACE ? false : resplitting;
				...
				double alt = hit ? hand.hitExval(theDeck,dealer) : hand.standExval(theDeck,dealer) ;
				hand.unhit(c1);
				double mean = hand.approxSplitPlay(theDeck,dealer,handResplit)-alt;
				...
				{	// remove one card
					...
					alt = hit ? hand.hitExval(theDeck,dealer) : hand.standExval(theDeck,dealer) ;
					hand.unhit(c1);
					double exval = hand.approxSplitPlay(theDeck,dealer,handResplit)-alt;
					os << "\t" << 100.*(exval-mean)*ndecks ;
					theDeck.restore(c2);
				}
