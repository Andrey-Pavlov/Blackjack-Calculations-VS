...
#pragma region PlayHand: Constructors and Destructors
...
PlayHand::PlayHand(char *cds,int firstCard,double betsize)
{	cards[0]=cds[0];
...
#pragma region PlayHand: Methods

// add 1 to repeat counter
void PlayHand::incrementRepeat(bool splitable,double betsize)
{	repeat++;
	bet += betsize;
	if(splitable)
	{	splits++;
...
bool PlayHand::removeAndGetWeight(Deck &deck,Dealer &dealer,double *wt)
{
	double cardwt,totalwt=1.;
...
bool PlayHand::removeAndGetNonsplitWeight(Deck &deck,Dealer &dealer,double *wt)
{	if(splits==repeat) return false;		// all hands are splits
	...
	// acceptable hand
	*wt = totalwt*(double)(repeat-splits);
	return true;
}
...
{	for(int i=1;i<=cards[0];i++)
	...
	hand->setDoubled(bet/(double)repeat);		// bet per hand
}
...
#pragma region PlayHand: Accessors
