...
#pragma region Deck: Constructors and Destructors
...
bool Deck::removeAndGetWeight(int card,double *weight)
{	if(nc[card]==0) return false;
	*weight=(double)nc[card]/(double)ncards;
	nc[card]--;
	ncards--;
	return true;
}
...
bool Deck::removeAndGetWeight(int card,double *weight,Dealer &dealer)
{	if(nc[card]==0) return false;
	int upcard = dealer.getUpcard();
	if(upcard==ACE || upcard==TEN)
	{	*weight=(double)nc[card]/(double)(ncards-1);
		if(card!=11-upcard)
			*weight*=(double)(ncards-nc[11-upcard]-1)/(double)(ncards-nc[11-upcard]);
	}
	else
		*weight=(double)nc[card]/(double)ncards;
	nc[card]--;
...
double Deck::probNotSplitCard(int splitCard,Dealer &dealer)
{
	double probSplit;
	int upcard = dealer.getUpcard();
	...
	{	// conditional probability of split given dealer has an ace or a ten
		probSplit = (double)nc[splitCard]/(double)(ncards-1);
		if(splitCard!=11-upcard)
			probSplit *= (double)(ncards-nc[11-upcard]-1)/(double)(ncards-nc[11-upcard]);
	}
	else
		probSplit = (double)nc[splitCard]/(double)ncards;
...
void Deck::probSplit234(int splitCard,Dealer &dealer,double *prob)
{
	double pns,pnm1s,pnm3ss,pnm2ss,pnm1ss;
	int upcard = dealer.getUpcard();
	
	if(upcard==ACE || upcard==TEN)
	{	pns = (double)nc[splitCard]/(double)(ncards-1);
		pnm1s = (double)nc[splitCard]/(double)(ncards-2);
		pnm1ss = (double)(nc[splitCard]-1)/(double)(ncards-2);
		pnm2ss = (double)(nc[splitCard]-1)/(double)(ncards-3);
		pnm3ss = (double)(nc[splitCard]-1)/(double)(ncards-4);
		if(splitCard != (11-upcard))
		{	double nmnj = (double)(ncards-nc[11-upcard]);
			pns *= (nmnj-1.)/nmnj;
			pnm1s *= (nmnj-2.)/(nmnj-1.);
	...
	{	pns = (double)nc[splitCard]/(double)ncards;
		pnm1s = (double)nc[splitCard]/(double)(ncards-1);
		pnm1ss = (double)(nc[splitCard]-1)/(double)(ncards-1);
		pnm2ss = (double)(nc[splitCard]-1)/(double)(ncards-2);
		pnm3ss = (double)(nc[splitCard]-1)/(double)(ncards-3);
	}
	
	// Let n=nonsplit, s=split, x=any
	prob[0] = (1.-pns)*(1.-pnm1s);						// nnxx (4) = P(2)
	double probns = (1.-pns)*pnm1s;
	double probsn = pns*(1-pnm1ss);
...
#pragma region Deck::Hand Hash Table
