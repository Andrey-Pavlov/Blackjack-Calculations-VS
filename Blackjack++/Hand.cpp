...
#pragma region Hand: Constructors and Destructors
...
#pragma region Hand: Expected Value Methods

// calculate the expected value of stand this hand against supplied dealer and deck
double Hand::standExval(Deck &deck,Dealer &dealer)
{	// loss if busted
...
double Hand::splitStandExval(Deck &deck,Dealer &dealer,int kout)
{	// loss if busted
	int index=getPlayerIndex();
	if(index>ExVal21) return -1.;
...
double Hand::hitExval(Deck &deck,Dealer &dealer)
{
	double exval=0,wt;
...
double Hand::doubleExval(Deck &deck,Dealer &dealer)
{
	double exval=0,wt;
...
#pragma region Hand::Splitting Calculations
...
void Hand::splitCalcs(Deck &deck,Dealer &dealer,int replitFlags,int ddFlags,double *results)
{
	int i;
...
double Hand::approxSplitPlay(Deck &deck,Dealer &dealer,bool resplit)
{
	double explay;
	if(resplit)
	{	// expected value with two split cards removed and conditioned
		// on second card of hand not being a split card
		double ex2 = approxSplitExval(deck,dealer,resplit,1);
...
double Hand::approxSplitExval(Deck &deck,Dealer &dealer,bool resplit,int kout)
{
	double exval=0,wt,pns;
...
void Hand::exactSplitCalcs(Deck &deck,Dealer &dealer,int maxSplitHands,int ddFlags,double *results)
{
	std::vector<Hand *>handsList(maxSplitHands);
	Hand **hands = &handsList[0];
	int i;
...
double Hand::exactSplitExval(Deck &deck,Dealer &dealer,Hand **hands,int &numSplitHands,int maxSplitHands)
{
	double exval=0.,wt;
	bool newHand;
...
#pragma region Hand::Splitting by Possible Hands

// Entry to handle bulk exact splitting calculations
// maxSplitHands=2 for no resplitting or no of hands enumerated
// ddFlags can be DDNone+DDAny+DD10OR11
// results is array of length 3
void Hand::handExactSplitCalcs(Deck &deck,Dealer &dealer,int maxSplitHands,int ddFlags,double *results)
{
	handset handList;
	std::vector<Hand *>handsList(maxSplitHands);
	Hand **hands = &handsList[0];
...
// hit a hand that has been split
double Hand::handExactSplitExval(Deck &deck,Dealer &dealer,Hand **hands,int &numSplitHands,int maxSplitHands,handset &handList)
{
	double exval=0.,wt;
...
#pragma region Hand::Collect Possible Hands
...
#pragma region Hand::Basic Strategy
...
double Hand::getDoubled(void) { return doubled; }
void Hand::setDoubled(double bet) { doubled=bet; }
double Hand::getBetPerHand(void) { return doubled; }
